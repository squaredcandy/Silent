...
#include "imgui_nodegrapheditor.h"
		...
		static void ImDrawListAddConvexPolyFilledWithVerticalGradient(ImDrawList *dl, const ImVec2 *points, const int points_count, ImU32 colTop, ImU32 colBot, float miny, float maxy)
		{
			...
			if (anti_aliased)
			{
				...
				for (int i0 = points_count - 1, i1 = 0; i1 < points_count; i0 = i1++)
				{
					...
					dl->_VtxWritePtr[0].pos = (points[i1] - dm); 
					dl->_VtxWritePtr[0].uv = uv; 
					dl->_VtxWritePtr[0].col = GetVerticalGradient(colTopf, colBotf, points[i1].y - miny, (float) height);        // Inner

					dl->_VtxWritePtr[1].pos = (points[i1] + dm); 
					dl->_VtxWritePtr[1].uv = uv; 
					dl->_VtxWritePtr[1].col = GetVerticalGradient(colTransTopf, colTransBotf, points[i1].y - miny, (float) height);  // Outer
					dl->_VtxWritePtr += 2;
			...
			else
			{
				...
				for (int i = 0; i < vtx_count; i++)
				{
					...
					dl->_VtxWritePtr[0].uv = uv; 
					dl->_VtxWritePtr[0].col = GetVerticalGradient(colTopf, colBotf, points[i].y - miny, (float) height);
					dl->_VtxWritePtr++;
				}
				for (int i = 2; i < points_count; i++)
				{
		...
		if (ImGui::BeginChild("GraphNodeChildWindow", ImVec2(0, 0), true))
		{
			...
			if (ImGui::BeginChild("scrolling_region", ImVec2(0, 0), true, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoScrollWithMouse))
			{
				...
				for (int node_idx = 0; node_idx < nodes.Size; node_idx++)
				{
					...
					{
						//if (nodeCallback)   {
						if (node->startEditingTime<0) node->startEditingTime = (float)ImGui::GetTime();
...
#       endif //NO_IMGUIHELPER_SERIALIZATION
	...
	{
		static T factor = T((180.0) / (3.1415926535897932384626433832795029));
		return factor;
	}
	template<typename T> inline static T GetDegsToRadians()
	{
		static T factor = T(3.1415926535897932384626433832795029) / T(180.0);
		return factor;
	}
